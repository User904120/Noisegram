...
/** Integration state and management boundary for the channel pinned-message-header setting. */
public final class TelegramHideChannelPinnedMessageHeaderSettingsGateway {
    ...
    interface StorageOperations {
        void load(TelegramHideChannelPinnedMessageHeaderStorage.LoadCallback callback);
    ...
    public TelegramHideChannelPinnedMessageHeaderSettingsGateway(
    ...
    void startInitialLoad() {
        try {
            storage.load(new TelegramHideChannelPinnedMessageHeaderStorage.LoadCallback() {
                @Override
    ...
    /** Read-only fail-open policy for the future channel pinned-header presentation gate. */
    public boolean shouldHideChannelPinnedMessageHeader() {
        try {
    ...
    public void getManagementState(Callback<ManagementState> callback) {
        ...
        post(operation, callback, terminal,
                "Cleargram storage queue rejected channel pinned-message-header state read");
    }

    public void setHideChannelPinnedMessageHeaderEnabled(boolean enabled, Callback<Void> callback) {
        Objects.requireNonNull(callback, "callback");
        AtomicBoolean terminal = new AtomicBoolean();
        Runnable operation = () -> {
            try {
                ...
                if (current.state != State.READY) {
                    throw new IllegalStateException("Channel pinned-message-header setting is not ready");
                }
        ...
        post(operation, callback, terminal,
                "Cleargram storage queue rejected channel pinned-message-header update");
    }
    ...
    private static StorageOperations createStorageOperations(
        ...
        TelegramHideChannelPinnedMessageHeaderStorage requiredStorage =
