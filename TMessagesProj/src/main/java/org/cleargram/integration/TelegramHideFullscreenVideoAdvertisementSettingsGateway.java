...
/** Integration state and management boundary for the fullscreen video advertisement setting. */
public final class TelegramHideFullscreenVideoAdvertisementSettingsGateway {
    ...
    interface StorageOperations {
        void load(TelegramHideFullscreenVideoAdvertisementStorage.LoadCallback callback);
    ...
    public TelegramHideFullscreenVideoAdvertisementSettingsGateway(
    ...
    void startInitialLoad() {
        try {
            storage.load(new TelegramHideFullscreenVideoAdvertisementStorage.LoadCallback() {
                @Override
    ...
    /** Read-only fail-open policy for the future VideoAds presentation gate. */
    public boolean shouldHideFullscreenVideoAdvertisement() {
        try {
    ...
    public void getManagementState(Callback<ManagementState> callback) {
        ...
        post(operation, callback, terminal,
                "Cleargram storage queue rejected fullscreen video advertisement state read");
    }

    public void setHideFullscreenVideoAdvertisementEnabled(boolean enabled, Callback<Void> callback) {
        Objects.requireNonNull(callback, "callback");
        AtomicBoolean terminal = new AtomicBoolean();
        Runnable operation = () -> {
            try {
                ...
                if (current.state != State.READY) {
                    throw new IllegalStateException("Fullscreen video advertisement setting is not ready");
                }
        ...
        post(operation, callback, terminal,
                "Cleargram storage queue rejected fullscreen video advertisement update");
    }
    ...
    private static StorageOperations createStorageOperations(
        ...
        TelegramHideFullscreenVideoAdvertisementStorage requiredStorage =
