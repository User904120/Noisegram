...
/** Shared SQLite storage boundary for the global Hide Reactions setting. */
public final class TelegramHideReactionsStorage {
    ...
    static final String HIDE_REACTIONS_KEY = "hide_reactions";

    private static final class StorageFailure extends RuntimeException {
        StorageFailure(Throwable cause) {
            super("Telegram Hide Reactions storage failure", cause);
        }
    }
    ...
    public TelegramHideReactionsStorage(
    ...
    TelegramHideReactionsStorage(WriteOperation testWriteOperation) {
        database = null;
        storageQueue = null;
    ...
    public void load(LoadCallback callback) {
        ...
        if (!state.compareAndSet(State.NEW, State.LOADING)) {
            throw new IllegalStateException("Hide Reactions storage load is only allowed from NEW state");
        }
        try {
            if (!storageQueue.postRunnable(() -> loadOnStorageQueue(callback))) {
                ...
                callback.onFailed(new StorageFailure(new IllegalStateException(
                        "Cleargram storage queue rejected Hide Reactions load")));
            }
    ...
    public void setEnabled(boolean enabled) {
        ...
        assertOwningQueue();
        try {
            database.upsertSettingValue(HIDE_REACTIONS_KEY, enabled ? "1" : "0");
    ...
    static boolean decodeStoredValue(String value) {
        ...
        throw new IllegalStateException("Invalid Hide Reactions setting value");
    }

    private void loadOnStorageQueue(LoadCallback callback) {
        assertOwningQueue();
        try {
            boolean enabled = decodeStoredValue(database.loadSettingValue(HIDE_REACTIONS_KEY));
            state.set(State.READY);
    ...
    private void assertReady() {
        if (state.get() != State.READY) {
            throw new IllegalStateException("Hide Reactions storage is not READY");
        }
    }
    ...
    private void assertOwningQueue() {
        ...
        if (handler == null || Thread.currentThread() != handler.getLooper().getThread()) {
            throw new IllegalStateException(
                    "Hide Reactions storage operation must run on the owning Cleargram storage queue");
        }
    }
}
