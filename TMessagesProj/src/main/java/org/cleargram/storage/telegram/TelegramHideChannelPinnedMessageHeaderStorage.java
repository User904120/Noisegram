...
/** SQLite boundary for the independent channel pinned-message-header setting. */
public final class TelegramHideChannelPinnedMessageHeaderStorage {
    ...
    static final String SETTING_KEY = "hide_channel_pinned_message_header";

    private static final class StorageFailure extends RuntimeException {
        StorageFailure(Throwable cause) {
            super("Channel pinned-message-header setting storage failure", cause);
        }
    }
    ...
    public TelegramHideChannelPinnedMessageHeaderStorage(
    ...
    TelegramHideChannelPinnedMessageHeaderStorage(WriteOperation testWriteOperation) {
        database = null;
        storageQueue = null;
    ...
    public void load(LoadCallback callback) {
        ...
        if (!state.compareAndSet(State.NEW, State.LOADING)) {
            throw new IllegalStateException(
                    "Channel pinned-message-header setting load is only allowed from NEW state");
        }
        try {
            if (!storageQueue.postRunnable(() -> loadOnStorageQueue(callback))) {
                ...
                callback.onFailed(new StorageFailure(new IllegalStateException(
                        "Cleargram storage queue rejected channel pinned-message-header setting load")));
            }
    ...
    private void assertReady() {
        if (state.get() != State.READY) {
            throw new IllegalStateException("Channel pinned-message-header setting storage is not READY");
        }
    }
    ...
    private void assertOwningQueue() {
        ...
        if (handler == null || Thread.currentThread() != handler.getLooper().getThread()) {
            throw new IllegalStateException(
                    "Channel pinned-message-header setting operation must run on the owning Cleargram storage queue");
        }
    }
}
